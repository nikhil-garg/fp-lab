...
template<typename ImageType, typename TransformType>
double getRegistrationError(typename ImageType::Pointer image,
	typename TransformType::Pointer transformA,
	typename TransformType::Pointer transformB
)
{
	typename ImageType::Pointer thresholdImage = OtsuFilter<ImageType>(image);
	itk::ImageRegionIteratorWithIndex<ImageType> It(thresholdImage, thresholdImage->GetLargestPossibleRegion());
	typedef typename ImageType::PointType  PointType;
	double error = 0;
	double N = 0;
	for (It.GoToBegin(); !It.IsAtEnd(); ++It)
	{
		if (It.Get() == 1) // Voxel is valid
		{
			PointType pt;
			PointType ptA;
			PointType ptB;
			thresholdImage->TransformIndexToPhysicalPoint(It.GetIndex(), pt);
			typename TransformType::Pointer transform = TransformType::New();
			ptA = transformA->TransformPoint(pt);
			ptB = transformB->TransformPoint(pt);
			error += ptA.EuclideanDistanceTo(ptB);
			++N;
		}
...
template<typename ImageType, typename PointSetType, typename TransformType>
typename TransformType::Pointer markerbasedRegistration(typename PointSetType::Pointer	fixedPoints,
	typename PointSetType::Pointer	movingPoints,
...
)
{
	...
	initializer->SetMovingImage( fixedImage );
	initializer->GeometryOn();
	initializer->InitializeTransform();  */
	// define metric:	
	typedef typename itk::EuclideanDistancePointSetToPointSetMetricv4<PointSetType, PointSetType> PointSetMetricType;
	typename PointSetMetricType::Pointer metric = PointSetMetricType::New();
	metric->SetFixedPointSet(fixedPoints);
	metric->SetMovingPointSet(movingPoints);
	metric->SetMovingTransform(transform);
	metric->SetVirtualDomain(fixedImage->GetSpacing(), fixedImage->GetOrigin(), fixedImage->GetDirection(),
		fixedImage->GetLargestPossibleRegion());
	//metric->SetVirtualDomainFromImage(fixedImage);
	metric->Initialize();
	// define optimizer:
	typedef typename itk::RegistrationParameterScalesFromPhysicalShift< PointSetMetricType > RegistrationParameterScalesFromShiftType;
